...
class YoutubeDownloader():
    ...
    def download_video(self):
        if self.video_url.get() == "":
            ...
            try:
                ...
                self.start_downloading = Label(self.root, text="Video downloading .....", fg="red", font=(
                    'verdana', 10, 'bold'), bg="white")
                self.start_downloading.place(x=40, y=10)

                self.stream.download(output_path=self.path, filename=None)

                self.progress = Progressbar(
                    self.root, orient=HORIZONTAL, length=250, mode='determinate')
                ...
                self.dow_details = ScrolledText(
                    self.root, width=30, height=3, font=('verdana', 8, 'bold'))
                self.dow_details.place(x=20, y=70)
                self.dow_details.insert(
                    END, f'{self.video_path.get()}\n {self.video.title}')

                self.dow_success = Label(self.root, text="Video downloaded successfully .....", fg="red", font=(
                    'verdana', 10, 'bold'), bg="white")
                ...
                messagebox.showerror(
                    "Error", "Unable to Download Video | Something went wrong !!")
    ...
    def download_playlist(self):
        if self.playlist_url.get() == "":
            ...
            try:
                ...
                self.start_downloading = Label(self.root, text="Playlist downloading .....", fg="red", font=(
                    'verdana', 10, 'bold'), bg="white")
                self.start_downloading.place(x=40, y=10)

                for self.video in self.playlist:
                    self.video.streams.get_highest_resolution().download(
                        output_path=self.path, filename=None)
                ...
                self.dow_details.place(x=20, y=70)
                self.dow_details.insert(
                    END, f'{self.playlist_path.get()}\n {self.video.title}')

                self.dow_success = Label(self.root, text="Playlist downloaded successfully .....", fg="red", font=(
                    'verdana', 10, 'bold'), bg="white")
                self.dow_success.place(x=10, y=120)
    ...
    def __init__(self):
        ...
        self.design1 = Label(self.root, bg="White", width=20)
        self.design1.place(x=0, y=45)

        self.date = Label(self.root, text=datetime.now(),
                          font=('verdana', 10, 'bold'), bg="white")
        self.date.place(x=140, y=45)

        self.design2 = Label(self.root, bg="White", width=20)
        ...
        self.frame1 = LabelFrame(self.root, text="Download Video", width=180, height=180, font=(
            'verdana', 10, 'bold'), bg="white", fg="red", borderwidth=5, relief=SUNKEN, highlightcolor="red", highlightbackground="red")
        self.frame1.place(x=10, y=80)

        self.v_url = Label(self.frame1, text="Paste url Here ...",
                           font=('verdana', 10, 'bold'), bg="white")
        self.v_url.place(x=20, y=2)

        self.video_url = Entry(
            self.frame1, width=24, relief=SUNKEN, borderwidth=2, bg="red", fg="black")
        self.video_url.place(x=10, y=30)

        self.v_path = Label(self.frame1, text="Select Path",
                            font=('verdana', 10, 'bold'), bg="white")
        self.v_path.place(x=10, y=60)

        self.video_path = Entry(
            self.frame1, width=15, relief=SUNKEN, borderwidth=2, bg="red", fg="black")
        self.video_path.place(x=10, y=90)

        self.file = Button(self.frame1, text="Browser", font=(
            'verdana', 8, 'bold'), relief=RAISED, bg="white", command=self.select_v_path)
        self.file.place(x=105, y=88)

        self.download_video = Button(self.frame1, text="Download", font=(
            'verdana', 9, 'bold'), relief=RAISED, bg="white", borderwidth=4, command=self.download_video)
        self.download_video.place(x=40, y=125)

        # =============== Palylist =======================

        self.frame2 = LabelFrame(self.root, text="Download Playlist", width=180, height=180, font=(
            'verdana', 10, 'bold'), bg="white", fg="red", borderwidth=5, relief=SUNKEN, highlightcolor="red", highlightbackground="red")
        self.frame2.place(x=310, y=80)

        self.p_url = Label(self.frame2, text="Paste url Here ...",
                           font=('verdana', 10, 'bold'), bg="white")
        self.p_url.place(x=20, y=2)

        self.playlist_url = Entry(
            self.frame2, width=24, relief=SUNKEN, borderwidth=2, bg="red", fg="black")
        self.playlist_url.place(x=10, y=30)

        self.p_path = Label(self.frame2, text="Select Path",
                            font=('verdana', 10, 'bold'), bg="white")
        self.p_path.place(x=10, y=60)

        self.playlist_path = Entry(
            self.frame2, width=15, relief=SUNKEN, borderwidth=2, bg="red", fg="black")
        self.playlist_path.place(x=10, y=90)

        self.playlist_file = Button(self.frame2, text="Browser", font=(
            'verdana', 8, 'bold'), relief=RAISED, bg="white", command=self.select_p_path)
        self.playlist_file.place(x=105, y=88)

        self.download_playlist = Button(self.frame2, text="Download", font=(
            'verdana', 9, 'bold'), relief=RAISED, bg="white", borderwidth=4, command=self.download_playlist)
        self.download_playlist.place(x=40, y=125)

        self.clear = Button(self.root, text="Clear", font=('verdana', 10, 'bold'),
                            bg="white", fg="red", padx=10, relief=RAISED, borderwidth=3, command=self.Clear)
        self.clear.place(x=220, y=195)

        self.quit = Button(self.root, text="Quit", font=('verdana', 10, 'bold'), bg="red",
                           fg="white", padx=15, relief=RAISED, borderwidth=3, command=self.Quit)
